...
class ColorThief(object):
    ...
    def __init__(self, image):
        ...
        :param image: opencv color image.
        """
        self.image = image
    ...
    def get_palette(self, color_count=5, quality=10):
        ...
        colors = []
        for y in range(h):
            for x in range(0,w,quality):
                r = self.image.item(y,x,2)
                g = self.image.item(y,x,1)
                b = self.image.item(y,x,0)
                if not (r > 250 and g > 250 and b > 250):
                    colors.append((r, g, b))
        
        cmap = MMCQ.quantize(colors, color_count)
        return cmap.palette
